...
%Note that if more information such as the underlying road networks are provided, the road trajectories that has the maximum likelihood to match visited tower sequences can also be used instead of the straight line trajectories.
...
\begin{figure*}
  \centering
  \subfigure[Single distance may fail\label{fig:illustrate_cases}]{\includegraphics[width=0.32\textwidth]{figures/illustrate_cases.pdf}}
  \subfigure[Voronoi diagram         \label{fig:voronoi}]{\includegraphics[width=0.32\textwidth]{figures/voronoi_illustrate.pdf}}
  \subfigure[Virtual boundaries      \label{fig:virtual}]{\includegraphics[width=0.32\textwidth]{figures/virtual_boundary.pdf}}
  \vspace{-0.1in}
...
%Since we do not have any information regarding which towers the user has visited in between, to calculate the distance lower bound of a virtual boundary, we instead use the shortest distance of all possible boundary pairs of $l_i$ and $l_j$ as the best estimate.
